...
public class RasterService
{
    public RasterService()
    {
        ...
        // IMPORTANTE: Força o GDAL (versão 3+) a usar a ordem Tradicional GIS (X=Longitude/Easting, Y=Latitude/Northing)
        // Sem isto, algumas projeções (como EPSG:4326 ou SIRGAS) podem inverter os eixos X e Y internamente
        // resultando num ecrã em branco ou satélites a aparecerem noutro lado do mundo durante o Warp.
    ...
    public string AnalisarArquivoRaster(string caminhoArquivo)
    {
        try
        {
            ...
            RasterBounds bounds = RasterGeometry.GetBounds(
                RasterGeometry.GetBoundaryPoints(geoTransform, largura, altura));

            return $"Raster Lido com Sucesso!\nResolução: {largura}x{altura} pixels\nCoordenadas:\nX Mínimo: {Math.Round(bounds.MinX, 2)}\nY Máximo: {Math.Round(bounds.MaxY, 2)}";
        }
        catch (Exception ex)
        {
    ...
    [Obsolete("Legacy preview path; the active map renderer uses LocalMapServer and preserves raster semantics.")]
    ...
    [Obsolete("Legacy crop path; the active map renderer uses LocalMapServer and preserves raster semantics.")]
