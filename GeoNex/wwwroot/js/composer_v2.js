...
document.addEventListener('keydown', function(e) {
    ...
    if (e.ctrlKey && e.key.toLowerCase() === 'c') {
        if (activeItem && dotnetHelper) {
            dotnetHelper.invokeMethodAsync('OnItemCopied', activeItem.id);
        }
    }
    if (e.ctrlKey && e.key.toLowerCase() === 'v') {
        if (dotnetHelper) {
            dotnetHelper.invokeMethodAsync('OnItemPasted');
        }
    }
});
...
window.composerUpdateItemProperty = function (id, propName, propValue) {
    ...
    if (propName === 'BorderColor' || propName === 'BorderWidth' || propName === 'HasBorder') {
        ...
        let w = parseInt(item.dataset.borderWidth || '1');
        ...
        if (hw) {
            item.style.border = `${w}px solid ${cw}`;
        } else {
...
window.renderLegendHTML = function(itm, cnt) {
    let title = itm.dataset.text || 'Legenda';
    let html = `<strong style="color:#000;">${title}</strong><br><br>`;
    
    try {
        if (itm.dataset.legendData) {
            let legendData = JSON.parse(itm.dataset.legendData);
            legendData.forEach(lItem => {
                if (lItem.IsVisible) {
                    html += `<div style="display:flex; align-items:center; margin-bottom:5px;">
                                <div style="width:15px;height:15px;background-color:${lItem.Color};border:1px solid ${lItem.BorderColor};margin-right:8px;min-width:15px;"></div>
                                <span style="font-size:12px;">${lItem.Name}</span>
                             </div>`;
                }
            });
        } else {
            html += `<div style="display:flex; align-items:center; margin-bottom:5px;"><div style="width:20px;height:20px;background:red;margin-right:8px;"></div> (Atualizando...)</div>`;
        }
    } catch (e) {
        console.error("Error parsing LegendItems", e);
    }
