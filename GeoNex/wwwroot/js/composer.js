...
window.composerInit = function (paperId) {
...
window.composerAddItem = function (type, text, x, y, w, h) {
    ...
    if (type === 'Map') {
        ...
        // Clona TODAS as camadas (Tile, Image, Vetores) do mapa principal para o compositor
        if (window.geonexMap) {
            window.geonexMap.eachLayer(function (layer) {
                try {
                    if (layer instanceof L.TileLayer.WMS) {
                        L.tileLayer.wms(layer._url, layer.options).addTo(composerLeafletMap);
                    } 
                    else if (layer instanceof L.TileLayer) {
                        L.tileLayer(layer._url, layer.options).addTo(composerLeafletMap);
                    } 
                    else if (layer instanceof L.ImageOverlay) {
                        L.imageOverlay(layer._url, layer._bounds, layer.options).addTo(composerLeafletMap);
                    }
                    else if (layer.toGeoJSON) {
                        // É uma camada vetorial (que no ecrã principal pode estar oculta pelo SkiaSharp).
                        // Vamos clonar o GeoJSON e forçar a visibilidade para o Composer!
                        let cloneOptions = Object.assign({}, layer.options);
                        
                        // Ignoramos a invisibilidade forçada do SkiaSharp
                        if (cloneOptions.opacity === 0 || cloneOptions.opacity === 'transparent') cloneOptions.opacity = 1;
                        if (cloneOptions.fillOpacity === 0) cloneOptions.fillOpacity = 0.4;
                        if (cloneOptions.color === 'transparent') cloneOptions.color = '#0ea5e9';
                        if (cloneOptions.fillColor === 'transparent') cloneOptions.fillColor = '#0ea5e9';
                ...
                } catch(e) { console.error("Erro a clonar layer:", e); }
            });
        }

        // Se não houver camadas base copiadas (falha ou mapa principal vazio), colocamos um fallback
        let hasBaseLayer = false;
        composerLeafletMap.eachLayer(function() { hasBaseLayer = true; });
        if (!hasBaseLayer) {
...
window.composerSetMapInteractionMode = function (isActive) {
